...
public class BlockLogServiceImpl implements BlockLogService {
    ...
    public Mono<BlockLog> create(BlockLog blockLog) {
        blockLog.setCreateTime(DateUtil.now());
        blockLog.setLocation(BaseAddressUtil.getCityInfo(blockLog.getIp()));
    ...
    @Override
    public Flux<BlockLog> findPages(BasePageAo request, BlockLog blockLog) {
        Query query = getQuery(blockLog);
        return BasePageableExecutionUtil.getPages(query, request, BlockLog.class, template);
    }
